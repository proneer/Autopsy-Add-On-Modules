...
sigDic={"exe":"\x4d\x5a", "dll":"\x4d\x5a"}
...
class SampleJythonFileIngestModuleFactory(IngestModuleFactoryAdapter):
    ...
    moduleName = "CheckingSignatureV01"
...
class SampleJythonFileIngestModule(FileIngestModule):
    ...
    def process(self, file):
        ...
        #slack files or no Extension files or no Size files are pass
        if (file.getName().lower().endswith("-slack") or
            file.getName()==file.getName().split('.')[-1] or 
            file.getSize()<8 ):
            return IngestModule.ProcessResult.OK

        #Extract File Extension
        ext=file.getName().split('.')[-1].lower()
		
        #Check if there is dictionary or not
        try:
            sigDic[ext]
        except:
            return IngestModule.ProcessResult.OK
        ...
        try:
            ...
            self.log(Level.INFO, "Extension Directory already exists " + temp_Dir)
			
        # Write out file to the temp directory
        # Save the DB locally in the temp folder. use file id as name to reduce collisions
        lclDbPath = os.path.join(temp_Dir, file.getName())
        ContentUtils.writeToFile(file, File(lclDbPath))
		
        #Oepn File
        temp_File=temp_Dir+"\\"+file.getName()
        f=open(temp_File,"rb")
        sig=f.read(8)
        ...
        if !sig.startswith(sigDic[ext]):
            ...
            att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME.getTypeID(), SampleJythonFileIngestModuleFactory.moduleName, "Got Files")
